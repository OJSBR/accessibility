...
class PluginTest extends TestCase
{
    ...
    {
        $this->assertTrue(is_subclass_of(AccessibilityBlockPlugin::class, \PKP\plugins\BlockPlugin::class));
        $reflection = new ReflectionClass(AccessibilityBlockPlugin::class);
        $parent = $reflection->getParentClass();
        foreach ($reflection->getMethods() as $method) {
            if ($method->getDeclaringClass()->getName() !== AccessibilityBlockPlugin::class || !$parent->hasMethod($method->getName())) {
                continue;
            }
    ...
    public function testTheAssetsAreFilesAndNotInlineCode(): void
    {
        ...
        $source = (string) file_get_contents($root . '/AccessibilityBlockPlugin.php');
